...
ENABLE QUERY REWRITE
AS
SELECT TITLE, GENRE, YEAR, MONTH, DAY, COUNT(USER_KEY)
...
GROUP BY GENRE, TITLE, YEAR, MONTH, DAY;
...
ENABLE QUERY REWRITE
AS
SELECT TITLE, COUNT(STREAM_DURATION) AS "Nb_d_ecoute"
FROM SPOTIFY_STREAM, TIME_DAY_DIM, MUSIC_DIM
WHERE TIME_DAY_DIM.TIME_DAY_KEY = SPOTIFY_STREAM.DATE_KEY
AND MUSIC_DIM.MUSIC_KEY = SPOTIFY_STREAM.MUSIC_KEY
AND TIME_HOUR BETWEEN EXTRACT(TIME_HOUR FROM CURRENT_TIMESTAMP)  AND (EXTRACT(TIME_HOUR FROM CURRENT_TIMESTAMP) - 1)
...
ENABLE QUERY REWRITE
AS
SELECT NAME, EMAIL, TITLE, GENRE SUM(STREAM_DURATION) AS "Nb_listening", PLAYLIST_DIM.TITLE, PLAYLIST_DIM.GENRE
