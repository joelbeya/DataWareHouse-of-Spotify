...
ENABLE QUERY REWRITE
AS
SELECT CONTINENT, COUNTRY, GENRE, TITRE, SUM(STREAM_DURATION) AS "streams_count"
...
-- ENABLE QUERY REWRITE
-- AS
-- SELECT TITLE, GENRE, YEAR, MONTH, DAY, TIME_HOUR, COUNT(USER_KEY)
...
-- GROUP BY GENRE, TITRE, YEAR, MONTH, DAY, TIME_HOUR;
...
-- AND MUSIC_DIM.MUSIC_KEY = SPOTIFY_STREAM.MUSIC_KEY
-- AND FULL_TIME BETWEEN EXTRACT(TIME_HOUR FROM CURRENT_TIMESTAMP)  AND (EXTRACT(TIME_HOUR FROM CURRENT_TIMESTAMP) - 1)
